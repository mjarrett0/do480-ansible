...
# This version fixes the deployment naming confusion by confirming the Deployment names
# generated by the Helm chart for BOTH the Etherpad app and the MySQL sub-chart.
set -e  # Exit on error
...
# CRITICAL FIX: Deployment Names based on common Helm conventions
MYSQL_DEPLOYMENT_NAME="${DB_RELEASE_NAME}-mysql"
ETHERPAD_DEPLOYMENT_NAME="${DB_RELEASE_NAME}-etherpad" # Based on the Pod name prefix
...
wait_for_deployment_ready() {
    ...
    if oc wait --for=condition=Available deployment/"$deployment_name" -n "$ns" --timeout="${timeout}s"; then
        ...
        oc get pods,deployments -n "$ns" # Show current status for diagnostics
        return 1
    fi
}
...
DB_HOST_SERVICE_ACTUAL="${MYSQL_DEPLOYMENT_NAME}"
